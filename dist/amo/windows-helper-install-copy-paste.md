...
No PowerShell command, ZIP extraction, execution-policy change, drive mapping, or `net use` command is required.

Verify the installer against `SHA256SUMS.txt` from the same GitHub release before running it. The installer is not Authenticode-signed yet.
