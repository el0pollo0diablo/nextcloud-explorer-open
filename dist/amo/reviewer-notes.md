...
Version 0.3.1 keeps the existing AMO/WebExtension ID required for updates to this listing:
...
2. Run `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\build-release.ps1 -Version 0.3.1`.
...
Version 0.3.1 obtains the current page URL from the content script and Firefox menu event before sending it to the native helper. This fixes missing `tab.url` values without adding the broad Firefox `tabs` permission. The native helper still requires HTTPS and validates the exact configured origin and Nextcloud Files route.

The extension does not use the Firefox `storage` permission. The server URL, username, and credential are managed by the local Windows helper. The app password is stored in Windows Credential Manager and is not sent to Firefox.
